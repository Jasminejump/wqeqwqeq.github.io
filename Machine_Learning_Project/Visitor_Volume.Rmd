---
title: "Group 20"
author: "Sean Pedersen, Cole Smith, Brooke Ealey, Zhehui Yuan"
date: "4/21/2020"
...
```
## Project Goal
We will try to use the [restaurant data](https://www.kaggle.com/c/recruit-restaurant-visitor-forecasting/overview) from Kaggle to predict the future volume of visitor for restaurants in Japan. We build a time series model by using *cluster* and *prophet package* to reach RMSE at 0.76

## Packge install
We install necessary packages for this project first 
...
We use Gower's method by creating **5** clusters or **2** clusters after we checked the plot. We compare these two choice of cluster number, and make final decisions. But before we run our model, will fisrt run the author's model from kaggle first in order to do comparison 
```{r}
...
```
#### Author's method
Blocks of code below contains function that runs 42 day forecast based on avg demand per cluster. This is an improvement upon Konrad Banachewicz's score (kaggle Notebook author) because our forecast is based off of the average demand of each
cluster rather than an average of the enture data set. This is essential because we only want to base our forecasts off of restaurants of similar revenue structure. It wouldn't make sense to forecast the performance of a 5 star restaurant based on the demand of the local coffee shop down the street. In short, we want to compare apples to apples.
...
#actual RMSE
(five_cluster_customer_variance<- sqrt(exp(five_cluster_RMSE))) 
#reverses the log and then takes the square root in order to 
